...
public:
  ...
  PosixAttachOperation(char *name) : AttachOperation(name) { set_socket(-1); }
};
...
int PosixAttachListener::init() {
  ...
  if (res == 0) {
    ...
    if (res == 0) {
      ...
      // is set. Default behavior on mac is that new files inherit the group
...
PosixAttachOperation *PosixAttachListener::read_request(int s) {
  ...
  const size_t max_len =
      (sizeof(ver_str) + 1) + (AttachOperation::name_length_max + 1) +
  ...
  do {
    ...
    assert(n <= checked_cast<ssize_t>(left),
           "buffer was too small, impossible!");
    buf[max_len - 1] = '\0';
    if (n == -1) {
...
void PosixAttachOperation::complete(jint result, bufferedStream *st) {
  ...
  if (rc == 0) {
    PosixAttachListener::write_fully(this->socket(), (char *)st->base(),
                                     st->size());
...
bool AttachListener::check_socket_file() {
  ...
  if (ret == -1) { // need to restart attach listener.
    ...
    // wait to terminate current attach listener instance...
    {
      ...
      while (AttachListener::transit_state(
                 AL_INITIALIZING, AL_NOT_INITIALIZED) != AL_NOT_INITIALIZED) {
        os::naked_yield();
      }
    }
...
bool AttachListener::is_init_trigger() {
  ...
  if (ret == 0) {
    ...
    if (os::Posix::matches_effective_uid_or_root(st.st_uid)) {
      ...
      log_debug(attach)(
          "File %s has wrong user id %d (vs %d). Attach is not triggered", fn,
          st.st_uid, geteuid());
    }
  }
  return false;
...
void AttachListener::abort() { listener_cleanup(); }

void AttachListener::pd_data_dump() { os::signal_notify(SIGQUIT); }
