...
static char* anon_mmap_aligned(char* req_addr, size_t bytes, size_t alignment) {
  ...
  if (start != nullptr) {
    if (req_addr != nullptr) {
      if (start != req_addr) {
        int res = ::munmap(start, extra_size);
        if (res != 0) {
          ErrnoPreserver ep;
      ...
      if (start_aligned > start) {
        int res = ::munmap(start, start_aligned - start);
        if (res != 0) {
          ErrnoPreserver ep;
      ...
      if (end_aligned < end) {
        int res = ::munmap(end_aligned, end - end_aligned);
        if (res != 0) {
          ErrnoPreserver ep;
...
static char* reserve_memory_special_huge_tlbfs(size_t bytes,
  ...
  if (!large_committed) {
    ...
    int res = ::munmap(small_start, small_size);
    if (res != 0) {
      ErrnoPreserver ep;
  ...
  if (!small_committed) {
    ...
    int res = ::munmap(aligned_start, large_bytes);
    if (res != 0) {
      ErrnoPreserver ep;
